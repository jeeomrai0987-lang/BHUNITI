...
class Mutation(Base):
    """A mutation (title-transfer) case sitting in a revenue officer's queue."""
    ...
    assigned_officer_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"), index=True, nullable=True)
    ...
    # The officer queue filters by status and orders by submission date.
    __table_args__ = (Index("ix_mutations_status_submitted", "status", "submission_date"),)
